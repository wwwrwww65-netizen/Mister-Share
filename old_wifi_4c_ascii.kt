...
class WiFiDirectAdvancedModule(reactContext: ReactApplicationContext) :
    ...
    fun connectToNetwork(ssid: String, password: String, promise: Promise) {
        Log.d("WiFiDirect", "???? Connecting to network: $ssid")
        
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                // Android 10+ (API 29+) - Use WifiNetworkSpecifier
                Log.d("WiFiDirect", "???? Using WifiNetworkSpecifier (Android 10+)")
                ...
                val networkCallback = object : ConnectivityManager.NetworkCallback() {
                    override fun onAvailable(network: Network) {
                        ...
                        Log.d("WiFiDirect", "??? Network available! Storing in NetworkHolder...")
                    ...
                    override fun onUnavailable() {
                        super.onUnavailable()
                        Log.e("WiFiDirect", "??? Network unavailable: $ssid")
                        NetworkHolder.clear()
                    ...
                    override fun onLost(network: Network) {
                        ...
                        Log.w("WiFiDirect", "?????? Network lost: $ssid")
                        NetworkHolder.clear()
                    }
                }
                ...
                Log.d("WiFiDirect", "???? Using legacy WifiConfiguration (Android 9-)")
                connectToNetworkLegacy(ssid, password, promise)
            }
        } catch (e: Exception) {
            Log.e("WiFiDirect", "??? Connection error: ${e.message}")
                        ...
                        if (connectedSsid == ssid || connectedSsid?.contains(ssid.take(20)) == true) {
                            ...
                            try {
                                ...
                                if (networkToBind != null) {
                                    ...
                                    if (!isWifi) {
                                        Log.w("WiFiDirect", "?? WARNING: Bound network is NOT WiFi! Socket routing may fail.")
                                    }
                                } else {
    ...
    private fun getGatewayIp(): String {
        return try {
            ...
            Log.w("WiFiDirect", "?? Could not detect gateway IP, using fallback")
    ...
    fun getConnectedGatewayIp(promise: Promise) {
        try {
            ...
            Log.w("WiFiDirect", "?? Could not detect gateway, returning standard fallback")
    ...
    fun startWifiScanMonitoring(promise: Promise) {
        try {
            if (!isReceiverRegistered) {
                ...
                Log.d("MisterShare", "??? WiFi Scan Receiver Registered")
            }
            promise.resolve(true)
        } catch (e: Exception) {
    ...
    fun stopWifiScanMonitoring(promise: Promise) {
        try {
            if (isReceiverRegistered) {
                ...
                Log.d("MisterShare", "???? WiFi Scan Receiver Unregistered")
            }
            promise.resolve(true)
        } catch (e: Exception) {
    ...
    private fun emitScanResults() {
        try {
            ...
            Log.d("MisterShare", "???? Emitted ${networks.size()} scan results")
     ...
     * Order: LocalOnlyHotspot ??? WiFi Direct 5GHz ??? WiFi Direct 2.4GHz ??? Legacy
     */
    @SuppressLint("MissingPermission")
    @ReactMethod
    ...
    private fun tryLocalOnlyHotspot(promise: Promise) {
        Log.d("WiFiDirect", "???? Level 1: Trying LocalOnlyHotspot...")
        Log.d("WiFiDirect", "???? Android SDK: ${Build.VERSION.SDK_INT}, hotspotReservation: ${hotspotReservation != null}")
        
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
            Log.d("WiFiDirect", "??? LocalOnlyHotspot not supported (Android < 8), trying WiFi Direct...")
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            if (androidx.core.content.ContextCompat.checkSelfPermission(reactApplicationContext, android.Manifest.permission.NEARBY_WIFI_DEVICES) != android.content.pm.PackageManager.PERMISSION_GRANTED) {
                Log.d("WiFiDirect", "??? NEARBY_WIFI_DEVICES permission missing, trying WiFi Direct...")
        ...
        if (!isGpsEnabled && !isNetworkEnabled) {
            Log.d("WiFiDirect", "??? Location disabled, trying WiFi Direct...")
        ...
        if (wifiManager == null) {
            Log.d("WiFiDirect", "??? WiFi service not available, trying WiFi Direct...")
        ...
        if (!wifiManager.isWifiEnabled) {
            Log.d("WiFiDirect", "?????? WiFi is disabled, attempting to enable...")
            try {
                @Suppress("DEPRECATION")
                wifiManager.setWifiEnabled(true)
        ...
        android.os.Handler(android.os.Looper.getMainLooper()).post {
            try {
                Log.d("WiFiDirect", "???? Calling startLocalOnlyHotspot on Main Thread...")
                
                wifiManager.startLocalOnlyHotspot(object : WifiManager.LocalOnlyHotspotCallback() {
                    override fun onStarted(reservation: WifiManager.LocalOnlyHotspotReservation?) {
                        Log.d("WiFiDirect", "???? LocalOnlyHotspot onStarted callback received")
                        hotspotReservation = reservation
                        ...
                        try {
                            ...
                            currentHotspotConfig = result
                            Log.d("WiFiDirect", "??? LocalOnlyHotspot SUCCESS! SSID: $cleanSsid")
                            promise.resolve(result)
                        } catch (e: Exception) {
                            Log.e("WiFiDirect", "?????? LocalOnlyHotspot config retrieval failed: ${e.message}")
                            val result = Arguments.createMap().apply {
                                putBoolean("success", true)
                    ...
                    override fun onStopped() {
                        Log.d("WiFiDirect", "???? LocalOnlyHotspot stopped")
                        hotspotReservation = null
                        currentHotspotConfig = null
                    }
                    ...
                    override fun onFailed(reason: Int) {
                        ...
                        Log.d("WiFiDirect", "??? LocalOnlyHotspot FAILED: $reasonStr, trying WiFi Direct...")
                        tryWifiDirect5GHz(promise, "LocalOnlyHotspot failed: $reasonStr")
                    }
                }, android.os.Handler(android.os.Looper.getMainLooper()))
            } catch (e: Exception) {
                Log.e("WiFiDirect", "??? LocalOnlyHotspot exception: ${e.message}")
                e.printStackTrace()
    ...
    private fun force5GHzUsingReflection(promise: Promise) {
        Log.d("WiFiDirect", "????? Attempting Reflection Magic for Android 8/9 5GHz...")
        try {
    ...
    private fun tryWifiDirect5GHz(promise: Promise, previousError: String) {
        Log.d("WiFiDirect", "???? Level 2: Trying WiFi Direct 5GHz...")
        
        if (manager == null || channel == null) {
            Log.d("WiFiDirect", "??? WiFi P2P not available, trying 2.4GHz...")
            tryWifiDirect2GHz(promise, "$previousError ??? WiFi P2P not available")
            return
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            try {
                ...
                manager?.createGroup(channel!!, config, object : WifiP2pManager.ActionListener {
                    ...
                    override fun onFailure(reason: Int) {
                        Log.d("WiFiDirect", "??? WiFi Direct 5GHz FAILED: $reason, trying 2.4GHz...")
                        tryWifiDirect2GHz(promise, "$previousError ??? 5GHz failed: $reason")
                    }
                })
            } catch (e: Exception) {
                Log.d("WiFiDirect", "??? WiFi Direct 5GHz exception, trying 2.4GHz...")
                tryWifiDirect2GHz(promise, "$previousError ??? 5GHz exception: ${e.message}")
            }
        } else {
            // Android 10 and below - skip to 2.4GHz/Legacy
            tryWifiDirect2GHz(promise, "$previousError ??? Android < 11 (no 5GHz API)")
        }
    }

    private fun tryWifiDirect2GHz(promise: Promise, previousError: String) {
        Log.d("WiFiDirect", "???? Level 3: Trying WiFi Direct 2.4GHz...")
        
        if (manager == null || channel == null) {
            tryWifiDirectLegacy(promise, "$previousError ??? WiFi P2P not available")
            return
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            try {
                ...
                manager?.createGroup(channel!!, config, object : WifiP2pManager.ActionListener {
                    ...
                    override fun onFailure(reason: Int) {
                        Log.d("WiFiDirect", "??? WiFi Direct 2.4GHz FAILED: $reason, trying Legacy...")
                        tryWifiDirectLegacy(promise, "$previousError ??? 2.4GHz failed: $reason")
                    }
                })
            } catch (e: Exception) {
                tryWifiDirectLegacy(promise, "$previousError ??? 2.4GHz exception: ${e.message}")
            }
        } else {
    ...
    private fun tryWifiDirectLegacy(promise: Promise, previousError: String) {
        Log.d("WiFiDirect", "???? Level 4: Trying WiFi Direct Legacy (auto band)...")
        
        if (manager == null || channel == null) {
            Log.d("WiFiDirect", "??? ALL METHODS FAILED!")
        ...
        try {
            manager?.createGroup(channel!!, object : WifiP2pManager.ActionListener {
                ...
                override fun onFailure(reason: Int) {
                    Log.d("WiFiDirect", "??? ALL METHODS FAILED! Final error: $reason")
    ...
    private fun getGroupInfoForFallback(promise: Promise, method: String, band: String) {
        ...
        manager?.requestGroupInfo(channel) { group ->
            if (group != null) {
                ...
                Log.d("WiFiDirect", "??? $method SUCCESS! SSID: $ssid, Band: $band")
                promise.resolve(result)
            } else {
                ...
                Log.d("WiFiDirect", "??? $method SUCCESS (with defaults)")
                promise.resolve(result)
            }
        }
    }
        ...
        when (intent.action) {
            WifiP2pManager.WIFI_P2P_CONNECTION_CHANGED_ACTION -> {
                ...
                if (networkInfo?.isConnected == true) {
                    ...
                    manager?.requestConnectionInfo(channel) { info ->
                        ...
                        Log.d("WiFiDirect", "??? P2P Connected! GroupOwner: ${info.isGroupOwner}, Address: ${info.groupOwnerAddress?.hostAddress}")
                    ...
                    Log.d("WiFiDirect", "??? P2P Disconnected")
                    sendEvent("onP2PConnectionChanged", params)
                }
            }
            ...
            WifiP2pManager.WIFI_P2P_PEERS_CHANGED_ACTION -> {
                ...
                manager?.requestPeers(channel) { peers ->
                    ...
                    Log.d("WiFiDirect", "???? Found ${peers.deviceList.size} P2P peers")
            ...
            WifiP2pManager.WIFI_P2P_STATE_CHANGED_ACTION -> {
                ...
                Log.d("WiFiDirect", "???? P2P State: ${if (enabled) "ENABLED" else "DISABLED"}")
    ...
    fun startP2PMonitoring(promise: Promise) {
        ...
        try {
            ...
            Log.d("WiFiDirect", "??? P2P Monitoring started")
            promise.resolve(true)
        } catch (e: Exception) {
            Log.e("WiFiDirect", "??? Failed to start P2P monitoring: ${e.message}")
    ...
    fun stopP2PMonitoring(promise: Promise) {
        ...
        try {
            ...
            Log.d("WiFiDirect", "???? P2P Monitoring stopped")
            promise.resolve(true)
        } catch (e: Exception) {
            Log.e("WiFiDirect", "??? Failed to stop P2P monitoring: ${e.message}")
