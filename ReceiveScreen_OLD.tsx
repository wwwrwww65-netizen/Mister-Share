...
const ReceiveScreen = ({ navigation, route }: any) => {
    ...
    useEffect(() => {
        ...
        const startBLEDiscovery = async () => {
            try {
                ...
                console.log('[ReceiveScreen] ظ£à BLE Discovery initialized');

                // 2. Start BLE Advertising (make this device discoverable)
                console.log('[ReceiveScreen] ≡اôة Starting BLE Advertising as:', deviceName);
                await BLEDiscovery.startAdvertising(deviceName, ssid, password);
                console.log('[ReceiveScreen] ظ£à BLE Advertising started - device is now discoverable!');

                // 3. Listen for connection requests
                connectionRequestUnsub = BLEDiscovery.onConnectionRequest((request) => {
                    console.log('[ReceiveScreen] ≡اجإ Received BLE Connection Request:', request);
                ...
                console.log('[ReceiveScreen] ≡اôة P2P Monitoring Started');

            } catch (e) {
                console.log('[ReceiveScreen] ظإî BLE Discovery Start Failed:', e);
            }
        };

        // Start BLE discovery when hotspot is ready
        if (ssid && !isCreating) {
            console.log('[ReceiveScreen] ظ£à Hotspot ready, starting BLE discovery...');
            startBLEDiscovery();
        } else {
            console.log('[ReceiveScreen] ظ│ Waiting for hotspot... ssid:', ssid, 'isCreating:', isCreating);
        }

        return () => {
            console.log('[ReceiveScreen] ≡ا¤ State Change Cleanup (Listeners only)');
    ...
    const handleApproveConnection = async () => {
        ...
        try {
            ...
            console.log('[ReceiveScreen] ظ£à Connection approved, client will receive WELCOME');
            ...
            console.log('[ReceiveScreen] ظإî Approval failed:', e);
    ...
    useEffect(() => {
        ...
        const createGroup = async () => {
            try {
                ...
                // ظـظـظـظـظـظـظـظـظـظـظـظـظـظـظـ UNIVERSAL FALLBACK SYSTEM (SHAREIT METHOD) ظـظـظـظـظـظـظـظـظـظـظـظـظـظـظـ
                // Tries: LocalOnlyHotspot ظْ WiFi Direct 5GHz ظْ 2.4GHz ظْ Legacy
                // This ensures ALL devices can create a group!

                console.log('≡اأ Starting Universal Fallback System...');

                // 1. Check if hotspot is already active (Re-entry optimization)
                let info: any = await WiFiDirectAdvanced.getHotspotCredentials();

                if (info) {
                    console.log('ظآ╗ي╕ Reusing existing active hotspot:', info);
                } else {
                ...
                console.log(`ظ£à Group created via ${info.method}:`, info);
                ...
                if (mounted) {
                    ...
                    for (let attempt = 1; attempt <= 3; attempt++) {
                        try {
                            console.log(`[ReceiveScreen] ≡اôة Starting Handshake Server (attempt ${attempt}/3)...`);
                            ...
                            if (isRunning) {
                                serverStarted = true;
                                console.log('[ReceiveScreen] ظ£à Native Handshake Server started and verified');
                                break;
                            } else {
                                console.warn('[ReceiveScreen] ظأبي╕ Server reported started but isRunning=false');
                            }
                        } catch (e: any) {
                            console.error(`[ReceiveScreen] ظإî Failed to start handshake server (attempt ${attempt}):`, e.message);
                            if (attempt < 3) {
                    ...
                    if (!serverStarted) {
                        console.warn('[ReceiveScreen] ظأبي╕ Handshake server failed to start - clients may not be notified');
                    }
                    ...
                    if (serverStarted) {
                        try {
                            ...
                            console.log('[ReceiveScreen] ≡اôة NSD service registered for discovery');
                            ...
                            NsdService.onServiceResolved((info: any) => {
                                ...
                                console.log('[ReceiveScreen] ≡اôة NSD: Client detected:', info.deviceName || 'Unknown', '@', clientIp);
                                ...
                                console.log('[ReceiveScreen] ظ£à Client detected via NSD:', clientIp);
                            });

                            // Also start discovery to find clients
                            await NsdService.startDiscovery();
                            console.log('[ReceiveScreen] ≡ا¤ NSD discovery started for client detection');
                        } catch (e) {
                    ...
                    TcpHandshakeService.onPeerConnected((peer: PeerInfo) => {
                        console.log('[ReceiveScreen] ≡اجإ Peer Connected:', peer.name, '@', peer.ip);
                        ...
                        console.log('[ReceiveScreen] ظ£à Peer IP set for bidirectional transfer:', peer.ip);
                    ...
                    TcpHandshakeService.onApprovalRequest((request: TcpApprovalRequest) => {
                        console.log('[ReceiveScreen] ≡اôï Approval Request from:', request.clientName);
                        SoundService.notification();
                    ...
                    console.log(`≡اأ Ready! Method: ${methodUsed}, SSID: ${ssidVal}`);
                }
        ...
        return () => {
            ...
            console.log('[ReceiveScreen] ≡اؤّ Final Cleanup: Stopping services');
                    ...
                    <>
                        ...
                        {!isCreating && (
                            <View style={styles.detailsContainer}>
                                <Text style={styles.detailLabel}>╪┤╪ذ┘â╪ر ╪د┘╪د╪ز╪╡╪د┘ (SSID):</Text>
                                <Text style={styles.ssidText}>{ssid}</Text>

                                <View style={styles.passwordSection}>
                                    <Text style={styles.detailLabel}>┘â┘┘à╪ر ╪د┘┘à╪▒┘ê╪▒:</Text>
                                    <View style={styles.passwordBox}>
                                        <Text style={styles.passwordText}>{password}</Text>
                                    </View>
                                    <Text style={styles.passwordHint}>
                                        ╪ث╪«╪ذ╪▒ ╪╡╪»┘è┘é┘â ╪ذ┘ç╪░┘ç ╪د┘┘â┘┘à╪ر ╪ث┘ê ╪»╪╣┘ç ┘è┘à╪│╪ص QR
                                    </Text>
                                </View>
                            </View>
                        )}
            ...
            >
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        <Text style={styles.modalTitle}>{t('connect_ui.connection_request', '╪╖┘╪ذ ╪د╪ز╪╡╪د┘')}</Text>
                        <Text style={styles.modalText}>
                            {t('connect_ui.device_wants_connect', { name: approvalRequest?.deviceName || 'Unknown Device', defaultValue: `${approvalRequest?.deviceName} ┘è╪▒┘è╪» ╪د┘╪د╪ز╪╡╪د┘` })}
                        </Text>
                        ...
                        <View style={styles.modalButtons}>
                            ...
                            >
                                <Text style={styles.rejectBtnText}>{t('connect_ui.reject', '╪▒┘╪╢')}</Text>
                            </TouchableOpacity>
                            ...
                            >
                                <Text style={styles.approveBtnText}>{t('connect_ui.approve', '┘à┘ê╪د┘┘é')}</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                </View>
